...
final class NamespaceDirectories {
     ...
     * later concatenated into a URI whose {@code getPath()} decodes percent-encoding, so an encoded {@code ".."}
     * would otherwise decode back into a parent-directory segment and let the destination path escape the
     * intended namespace subtree.
     */
    static String normalize(String rendered) {
        if (rendered == null) {
            return "";
        }
        var segments = Arrays.stream(rendered.trim().split("/"))
    ...
    // concatenated into a destination URI. Decoding here (a single pass, like getPath()) catches encoded traversal
    // such as `%2e%2e` that a raw string check would miss.
