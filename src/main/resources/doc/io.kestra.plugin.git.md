...
The plugin covers two directions. To run code from a repository, use `Clone` inside a `WorkingDirectory` — cloned files are then available to all subsequent tasks in that working directory. `depth: 1` (the default) gives a shallow clone; set `commit` to check out a specific SHA or `tag` to check out a tag, both of which disable shallow cloning. To keep Kestra in sync with a Git repository, use `SyncFlows` to pull flow definitions and `SyncNamespaceFiles` to pull namespace files from a branch into a target namespace; set `delete: true` on either to remove resources that no longer exist in Git. `SyncNamespaceFiles` creates its target `namespace` automatically (best-effort, skipped in `dryRun`) if it doesn't exist yet, provided the Kestra API is reachable and authenticated (see Authentication above) — without a resolvable authentication it logs a warning and still syncs the files, but the namespace stays unregistered. To push Kestra configuration back to version control, use `PushFlows` for flows and `PushNamespaceFiles` for namespace files.

### Scoping namespace files to a subfolder

`SyncNamespaceFiles` and `PushNamespaceFiles` accept `namespaceDirectory` (default `/`, i.e. no scoping) to map a subfolder of a namespace to the root of `gitDirectory`. On sync, `/shared-scripts` writes a Git file `foo.py` to `<namespace>:/shared-scripts/foo.py`; on push, the prefix is stripped, so a Namespace File `/shared-scripts/foo.py` is pushed as `foo.py`. Two gotchas: `namespaceDirectory` also scopes `delete` (files outside the prefix are left untouched, even on the first scoped run), and on push the `files` glob still matches the full namespace-relative path, independently of the prefix. Values with `..` path segments are rejected.
